...
public class UIManager
{
    ...
    public T MakeSubItem<T>(Transform parent = null, string name = null) where T : UI_Base
    {
        ...
        GameObject go = Managers.Resources.Instantiate($"sh/UI/SubItem/{name}");
        if (parent != null)
    ...
    // 스크립트 수정 HP 텍스트 
    // 03.31
    public T MakeHpText<T>(Transform parent = null, string name = null) where T : UI_Base
    {
        if (string.IsNullOrEmpty(name))
            name = typeof(T).Name;

        GameObject go = Managers.Resources.Instantiate($"UI/TextHp/{name}");
        if (parent != null)
            go.transform.SetParent(parent);
    ...
    public T ShowSceneUI<T>(string name = null , string user = "sh") where T : UI_Scene
    {
        if (string.IsNullOrEmpty(name))
            name = typeof(T).Name;
    ...
    public T ShowSceneObject<T>(string name = null , string user = "sh") where T : MonoBehaviour
    {
        if (string.IsNullOrEmpty(name))
            name = typeof(T).Name;

        GameObject go = Managers.Resources.Instantiate($"{user}/Obj/Scene/{name}");
