...
public class UI_PlayerName : UI_Scene
{
    ...
    private void Start()
    {
        ...
        NameText.GetComponent<Text>().text = map.Player.transform.parent.name; // 이름 텍스트를 이걸로 설정해주고
        PlayerName();
    ...
    void PlayerName()
    {
        ...
        Vector2 screenPoint = RectTransformUtility.WorldToScreenPoint(Camera.main, map.Player.transform.parent.position); // 여기서 계산이 끝나는게 아니고 
